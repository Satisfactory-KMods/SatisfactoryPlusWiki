...
export function useRefDelay<T = any>(
	value: T | Ref<T>,
	onChange: (value: UnwrapRef<T>) => any = () => {},
	timeout = 500
) {
	const refValue: any = isRef(value) ? value : ref(value);
	const cached = ref(_.cloneDeep(refValue.value)!);
	...
	watch(
		...
		() => {
			cached.value = _.cloneDeep(unref(refValue.value))!;
		},
		{ deep: true }
	);
